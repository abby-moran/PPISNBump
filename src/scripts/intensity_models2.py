...
def log_dNdm(m, z, a, b, c, mpisn, mpisndot, mbhmax, sigma, fpl,
    ...
    """
    Compute log(dN/dm) at arbitrary (m, z), including:
    - Evolving PISN features
    - High-mass power-law tail
    - Smooth low-mass taper
    """
    m = jnp.atleast_1d(m)
    z = jnp.atleast_1d(z)
    nm, nz = m.shape[0], z.shape[0]

    # Prepare mass and redshift meshgrid
    m_grid = m[:, None]  # (nm, 1)
    z_grid = z[None, :]  # (1, nz)

    # Evaluate redshift-dependent mpisn(z) and mbhmax(z)
    z_array = jnp.expm1(jnp.linspace(jnp.log(1), jnp.log(1 + zmax), 30))  # (30,)
    mpisn_z = jnp.asarray(mpisn) + mpisndot * (1 - 1 / (1 + z_array))     # (30,)
    mbhmax_z = mpisn_z + (mbhmax - mpisn)                                 # (30,)

    # Interpolate PISN component across redshift
    log_dNdm_vals_z = log_dNdm_pisn(a, b, mpisn_z, mbhmax_z, sigma, m)  # (30, nm)
    log_dNdm_vals = jax.vmap(lambda zi: jnp.interp(zi, z_array, log_dNdm_vals_z[:, :], left=-jnp.inf, right=-jnp.inf), out_axes=1)(z)  # (nm, nz)

    # Apply smooth taper below mbh_min
    log_window = mmin_log_smooth_turnon(m_grid, delta_m=delta_m, mmin=mbh_min)  # (nm, 1)

    # Apply max BH mass cutoff
    max_m_grid = 100.0
    log_dNdm_vals = jnp.where(m_grid >= max_m_grid, -jnp.inf, log_dNdm_vals)

    # Compute power-law tail
    mbhmax_interp = jnp.interp(z, z_array, mbhmax_z)  # shape (nz,)
    log_dNdm_at_max = log_dNdm_pisn(a, b, mbhmax_interp, mbhmax_interp, sigma, mbhmax_interp).squeeze()  # shape (nz,)

    # Compute power-law extrapolation
    m_broadcast = m_grid  # (nm, 1)
    mbhmax_broadcast = mbhmax_interp[None, :]  # (1, nz)
    log_tail = -c * jnp.log(m_broadcast / mbhmax_broadcast)
    log_tail += jnp.log(fpl) + log_dNdm_at_max[None, :]
    log_tail += log_smooth_turnon(m_broadcast, mbhmax_broadcast)

    # Add logaddexp between base and power-law tail
    log_dNdm_total = jnp.logaddexp(log_dNdm_vals, log_tail)

    # Add smooth turn-on at low mass
    log_dNdm_total += log_window

    return log_dNdm_total  # shape (nm, nz)
